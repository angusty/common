...
class usercontrol extends base {
	...
	function onmodopenid(){
		$this->init_input();
		$username = $this->input('username');
		$openid = $this->input('openid');
		$is_weixinlogin = $this->input('is_weixinlogin');
		return $status = $_ENV['user']->modopenid_user($username,$openid,$is_weixinlogin);

	}

	function oncheckopenid(){
		$this->init_input();
		$openid = $this->input('openid');
		return $status = $_ENV['user']->checkopenid_user($openid);
	}

	function oncheckuser(){
		$this->init_input();
		$username = $this->input('username');
		return $status = $_ENV['user']->checkusername_user($username);
	}
	...
	function onupdate_member_by_uid()
	{
		$this->init_input();
		$uid = $this->input('uid');
		$username = $this->input('username');
		return $status = $_ENV['user']->updateMember($uid, $username);
	}
}
