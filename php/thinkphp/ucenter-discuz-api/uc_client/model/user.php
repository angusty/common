...
class usermodel {
	...
	function get_user_by_username($username,$regip='') {
		$arr = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE username='$username'");
		if($arr)
		{
			$loginip  = empty($regip) ? $this->base->onlineip : $regip;
			$lastlogintime = time();
			$this->db->query("UPDATE ".UC_DBTABLEPRE."members SET lastloginip='$loginip', lastlogintime='$lastlogintime' WHERE username='$username'");
		}
		return $arr;
	}
	...
	function check_username($username) {
		...
		$reg =  "/\s+|^c:\\con\\con|[%,\*\"\s\<\>\&]|$guestexp/is";
		$phone_reg = '/^1[34578]{1}\d{9}$/';
		if($len > 15 || $len < 3 || preg_match($reg, $username) || !preg_match($phone_reg, $username)) {
			return FALSE;
		} else {
			return TRUE;
		}
	...
	function add_user($username, $password, $email, $openid, $uid = 0, $questionid = '', $answer = '', $regip = '') {
		$regip = empty($regip) ? $this->base->onlineip : $regip;
		$salt = substr(uniqid(rand()), -6);
		$password = md5(md5($password).$salt);
		$sqladd = $uid ? "uid='".intval($uid)."'," : '';
		$sqladd .= $questionid > 0 ? " secques='".$this->quescrypt($questionid, $answer)."'," : " secques='',";
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."members SET $sqladd username='$username', password='$password', email='$email', openid='$openid', regip='$regip', regdate='".$this->base->time."', salt='$salt'");
		$uid = $this->db->insert_id();
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."memberfields SET uid='$uid'");


		  // BEGIN
        $this->db->query("INSERT INTO `bbsyiweimi`.pre_common_member SET uid='$uid', username='$username', password='$password', email='$email', adminid='0', groupid='10', regdate='".$this->base->time."', credits='0', timeoffset='9999'");
        $this->db->query("INSERT INTO `bbsyiweimi`.pre_common_member_status SET uid='$uid', regip='$regip', lastip='$regip', lastvisit='".$this->base->time."', lastactivity='".$this->base->time."', lastpost='0', lastsendmail='0'");
        $this->db->query("INSERT INTO `bbsyiweimi`.pre_common_member_profile SET uid='$uid'");
        $this->db->query("INSERT INTO `bbsyiweimi`.pre_common_member_field_forum SET uid='$uid'");
        $this->db->query("INSERT INTO `bbsyiweimi`.pre_common_member_field_home SET uid='$uid'");
        $this->db->query("INSERT INTO `bbsyiweimi`.pre_common_member_count SET uid='$uid', extcredits1='0', extcredits2='0', extcredits3='0', extcredits4='0', extcredits5='0', extcredits6='0', extcredits7='0', extcredits8='0'");
        // END
	...
	function modopenid_user($username, $openid,$is_weixinlogin) {
		    if($is_weixinlogin)
		    {
		    	$data = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE openid='$openid'");
				if($data)
				{
					return 99;
				}
				 ...
				 {
				 	$this->db->query("UPDATE ".UC_DBTABLEPRE."members SET openid='$openid' WHERE username='$username'");
				 }
		    }
		    else
		    {
		    	$list = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE openid='$openid'");
		    	if($list)
		    	{
		    		$res = $this->db->query("UPDATE ".UC_DBTABLEPRE."members SET openid='$openid' WHERE username='$username'");
		    		if($res)
		    		{
		    			$uid = $list['uid'];
		    			$this->db->query("DELETE FROM ".UC_DBTABLEPRE."members WHERE uid ='$uid'");
		    		}
		    	}
		    }
			return $this->db->affected_rows();
	...
	//根据uid 修改 ucenter用户名
	function updateMember($uid, $username) {
		$data = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE uid='{$uid}'");
		if($data) {
	...
	function checkopenid_user($openid){
		return  $data = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE openid='$openid'");
	}

	function checkusername_user($username){
		return  $data = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE username='$username'");
	}
